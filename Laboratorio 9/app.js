...
app.get("/:id", async (req, res) => {
  const obj = await pokemon.get(req.params.id);
  console.log(obj)
  // res.json(response);
  res.send(`<body><h2 style="
    ...
    font-family: monospace;
    color: darkslateblue;">
    <li>ID----> ${obj.pokemon.id}</li>
    <li>NamePokemon---->${obj.pokemon.name}</li>
    <li>Weight---->${obj.pokemon.weight}</li>
    <li>Height---->${obj.pokemon.height}</li>
    <li>AilitiesList---->${obj.pokemon.abilities}</li>
    <li>EvolestoList---->${obj.pokemon.evolves_to}</li>
    </ul></body>`)
});
app.get('/pokemon/json/:name', async function (req, res) {
    const pokemonId = req.params.name;
    res.json(await getPoke(pokemonId))
})
app.get('/pokemon/list/:name', async function (req, res) {
    const pokemonId = req.params.name;
    const obj =  await getPoke(pokemonId)
